	...
	if (typeof define === 'function' && define.amd) {
		...
		(function($, undefined) {
			...
			var instance_counter = 0, ccp_node = false, ccp_mode = false, ccp_inst = false, themes_loaded = [], src = $(
					...
					.createElement('LI'), _temp1, _temp2, _temp3, _temp4, _temp5;
			...
			_temp3 = _d.createElement('SPAN');
			_temp3.className = 'jstree-info c';
			_temp4 = _d.createElement('SPAN');
			_temp4.className = 'jstree-info a';
			_temp5 = _d.createElement('SPAN');
			_temp5.className = 'jstree-info m';
			_temp1.appendChild(_temp2);
			_node.appendChild(_temp1);
			_node.appendChild(_temp3);
			_node.appendChild(_temp4);
			_node.appendChild(_temp5);
			_temp1 = _temp2 = null;
			...
			$.jstree.core.prototype = {
				 ...
				 *            conplete, the function is executed in the
				...
				_append_json_data : function(dom, data, cb) {
					...
					var w = null, args = {
						...
						var dat = data.dat, par = data.par, chd = [], dpc = [], add = [], df = data.df, t_id = data.t_id, t_cnt = data.t_cnt, m = data.m, p = m[par], sel = data.sel, tmp, i, j, rslt, parse_flat = function(
							...
							if (d && d._id) {
								tmp.id = d._id.toString();
							}
							if (d && d.text) {
								tmp.text = d.text;
							}
							console.log(d.createdTime != undefined);
							if (d && d.createdTime && d.createdTime != undefined) {
								tmp.additional1 = new Date(d.createdTime)
										.toLocaleString();
							} else{
								tmp.additional1 = '';
							}
							
							if (d && d.accessedTime && d.accessedTime != undefined) {
								tmp.additional2 = "|"
										+ new Date(d.accessedTime)
												.toLocaleString();
							} else{
								tmp.additional2 = '';
							}
							if (d && d.lastModified  && d.lastModified != undefined) {
								tmp.additional3 = "|"
				...
				redraw_node : function(node, deep, is_callback) {
					...
					if ((obj.icon && obj.icon !== true) || obj.icon === false
							|| obj.original.extention) {
						if (obj.icon === false) {
							node.childNodes[1].childNodes[0].className += ' jstree-themeicon-hidden';
						} else if (obj.original.extention) {
							node.childNodes[1].childNodes[0].className = 'jstree-themeicon-custom'
									+ obj.original.extention;

						} else if (obj.original.extention === false
								&& obj.icon.indexOf('/') === -1
								&& obj.icon.indexOf('.') === -1) {
					...
					node.childNodes[2].innerHTML += obj.additional1;
					node.childNodes[3].innerHTML += obj.additional2;
					node.childNodes[4].innerHTML += obj.additional3;
					// if(obj.data) { $.data(node, obj.data); } // always work
					// with node's data, no need to touch jquery store
				...
				edit : function(obj, default_text) {
					 ...
					 * ai.width() * ai.length,
					 */
					t = typeof default_text === 'string' ? default_text : this
							.get_text(obj), h1 = $("<" + "div />", {
						css : {
							"position" : "absolute",
							"top" : "-200px",
						...
						"blur" : $.proxy(function() {
							var i = s.children(".jstree-rename-input"), v = i
									.val();
							if (v === "") {
								v = t;
							}
			...
			$.vakata.attributes = function(node, with_values) {
				...
				if (node && node.attributes) {
					$.each(node.attributes,
							function(i, v) {
								if ($.inArray(v.nodeName.toLowerCase(), [
										'style', 'contenteditable', 'hasfocus',
										'tabindex' ]) !== -1) {
									return;
								}
								if (v.nodeValue !== null
			...
			$.jstree.plugins.checkbox = function(options, parent) {
				this.bind = function() {
					...
					if (this.settings.checkbox.cascade.indexOf('undetermined') !== -1) {
						this.element
								.on(
										'changed.jstree move_node.jstree copy_node.jstree redraw.jstree open_node.jstree',
										$
												.proxy(
														function() {
															// only if
										...
										$
												.proxy(
														function(e, data) {
															...
															this._data.core.selected = $.vakata
																	.array_unique(this._data.core.selected);
														}, this))
								.on(
										'select_node.jstree',
										$
												.proxy(
														function(e, data) {
															var obj = data.node, m = this._model.data, par = this
																	...
																	.get_node(
																			obj,
																			true), i, j, c, tmp, s = this.settings.checkbox.cascade;
												...
												.proxy(
														function(e, data) {
															var obj = data.node, dom = this
																	.get_node(
																			obj,
																			true), i, j, tmp, s = this.settings.checkbox.cascade;
															if (obj
																	&& obj.original
																	&& obj.original.state
																...
																if ((s
																		...
																		&& (s
																				...
																				.inArray(
																						...
																						obj.parents) === -1)) {
																	tmp
																			.push(this._data.core.selected[i]);
																}
															}
															this._data.core.selected = $.vakata
																	.array_unique(tmp);
										...
										$
												.proxy(
														function(e, data) {
															...
															// handler)
															var p = this
																	.get_node(data.parent), m = this._model.data, i, j, c, tmp;
															while (p
																	&& p.id !== '#') {
																c = 0;
																for (
				...
				this.activate_node = function(obj, e) {
					if (this.settings.checkbox.whole_node
			...
			$.jstree.defaults.search = {
				 ...
				 * with the request. The expected result is a JSON array with
				 * nodes that need to be opened so that matching nodes will be
				 * revealed. Leave this setting as `false` to not query the
				 * server. You can also set this to a function, which will be
				 * invoked in the instance's scope and receive 2 parameters -
				 * the search string and the callback to call with the array of
				 * nodes to load.
				 * 
				...
				/**
				 * Indicates if the tree should be filtered to show only
				 * matching nodes (keep in mind this can be a heavy on large
				 * trees in old browsers). Default is `false`.
				 * 
			 ...
			 * ### Sort plugin
			 * 
			 * Autmatically sorts all siblings in the tree according to a
			 * sorting function.
			 */
			 ...
			 *  * `max_children` the maximum number of immediate children this
			 * node type can have. Do not specify or set to `-1` for unlimited. *
			 * `max_depth` the maximum number of nesting this node type can
			 * have. A value of `1` would mean that the node can have children,
			 * but no grandchildren. Do not specify or set to `-1` for
			 * unlimited. * `valid_children` an array of node type strings, that
			 * nodes of this type can have as children. Do not specify or set to
			 * `-1` for no limits. * `icon` a string - can be a path to an icon
			 * or a className, if using an image that is in the current
			 * directory use a `./` prefix, otherwise it will be detected as a
			 * class. Omit to use the default icon from your theme.
			 * 
			 * There are two predefined types:
			 *  * `#` represents the root of the tree, for example
			 * `max_children` would control the maximum number of root nodes. *
